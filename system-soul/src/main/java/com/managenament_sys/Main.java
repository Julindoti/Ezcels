...
import com.managenament_sys.service.APIService;
...
public class Main extends Application {
	...
	public void start (Stage primaryStage) throws Exception{
		FXMLLoader fxml = new FXMLLoader(getClass().getResource("/fxml/StartPage.fxml"));
		Parent root= fxml.load();
		Scene primary= new Scene(root);
		primaryStage.setScene(primary);
		primaryStage.setTitle("Ezcels");
		primaryStage.show();		
	...
	public static void main(String[] args) {
       ...
       ConfigurableApplicationContext context= builder.run(args);
		...
		try {
			...
			Gastos gasto1 = new Gastos("LOTE-1B", 17.00, new Date(), 1500, 10);
			...
			DatabaseService.getAllUsuarios();// Save to spreadsheet
